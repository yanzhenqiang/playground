...
class BaselinePolicy:
  ...
  def __init__(self):
    ...
    self.output_state = np.zeros(self.nOutput)
    self.prevoutput_state = np.zeros(self.nOutput)
  ...
  def reset(self):
    ...
    self.prevoutput_state = np.zeros(self.nOutput)
  def _forward(self):
    self.prevoutput_state = self.output_state
    self.output_state = np.tanh(np.dot(self.weight, self.inputState)+self.bias)
  def _set_input_state(self, obs):
    ...
    self.inputState[self.nGameInput:] = self.output_state
...
class Game:
  ...
  def step(self):
    ...
    if self.delayScreen.status():
      self.ball.apply_acceleration(0, GRAVITY)
      self.ball.limit_speed(0, MAX_BALL_SPEED)
      self.ball.move()
    ...
    if (self.ball.is_colliding(self.fenceStub)):
...
class SlimeVolleyEnv(gym.Env):
  ...
  def getObs(self):
    if self.from_pixels:
      ...
      obs = self.game.agent_right.get_observation()
    return obs
  ...
  def step(self, action, otherAction=None):
    ...
    if self.atari_mode:
      ...
      otherAction = self.discrete2box(otherAction)
    ...
    if self.multiagent:
      if self.from_pixels:
        ...
        otherObs = self.game.agent_left.get_observation()
...
register(
    id='SlimeVolley-v0',
    entry_point='rebuildergym.slimevolley:SlimeVolleyEnv'
)

register(
    id='SlimeVolleyNoFrameskip-v0',
    entry_point='rebuildergym.slimevolley:SlimeVolleyAtariEnv'
)
